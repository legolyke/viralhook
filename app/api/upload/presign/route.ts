...
import { validateFileFormat, validateFileSize, validateDuration } from '@/lib/upload-validator'
...
export async function POST(request: Request) {
  ...
  const { fileName, fileType, fileSize, durationSeconds, title } = body as {
    fileName: string
    fileType: string
    fileSize: number
  ...
  const sizeCheck = validateFileSize(fileSize, plan)
  if (!sizeCheck.valid) {
